...
"type": "LABEL",
"name": "warningCartId",
"displayName": "\u003cb\u003eBy default, the bridge can\u0027t retrieve any information to fuel \u0027cart_id\u0027
property. This property is mandatory for most of Piano Analytics ecommerce events.\u003cbr /\u003e\nIt is highly
recommended to add a mapping for this property.\u003c/b\u003e\u003cbr /\u003e\nWe will however retrieve the
\u0027transaction_id\u0027 to use as \u0027cart_id\u0027 if no \u0027cart_id\u0027 has been mapped, even if this is not
...
setPropObject[propPrefix(prop.setPropKey, prop.setPropType)] = propTypeCast(prop.setPropValue, (prop.setPropType ===
'auto'));
}
});
...
let finalEvents = [];
const customEventsMapping = data.ecomEventsMapping && makeTableMap(data.ecomEventsMapping, 'ecommerceEventInput',
'ecommerceEventOutput');
const customProductsMapping = data.ecomEventsMapping && makeTableMap(data.ecomEventsMapping, 'ecommerceEventOutput',
'ecommerceProductEventsOutput');
const customPropsMapping = data.ecomPropsMapping && makeTableMap(data.ecomPropsMapping, 'ecommercePropInput',
'ecommercePropOutput');
const constantProps = {};
...
for (var productMapping in customProductsMapping) { productsMapping[productMapping] =
customProductsMapping[productMapping]; }
}
...
for (let index = 0; index < ecommerceDatalayer.items.length; index++) { if (ecommerceDatalayer.items[index].quantity)
  totalQuantity +=makeInteger(ecommerceDatalayer.items[index].quantity); let productEventname="" ; // producing
  complementary products events for (var productEvent in productsMapping) { if (mappedEventname===productEvent)
  productEventname=productsMapping[productEvent]; } // any "product.xxx" event should inherit from items props if
  (isAutoItemsEvent) productEventname=mappedEventname; if (isOnsiteadEvent) { for (var mappedProp in ecomPropsMapping) {
  if(checkVarPrefix(ecomPropsMapping[mappedProp], ['onsitead'], '_' )) {
  ecommerceDatalayer.items[index][mappedProp]=ecommerceDatalayer[mappedProp]; } } } if (productEventname !=="" )
  finalEvents.push({ "name" : productEventname, "data" : mapProperties(ecommerceDatalayer.items[index],
  itemPropsMapping, constantProps) }); } } // automatically calculate "cart_quantity" property if (totalQuantity> 0)
  ecomPropsWithoutItems.cart_quantity = totalQuantity;

  if (!isAutoItemsEvent) {
  finalEvents.push({ "name": mappedEventname, "data": mapProperties(ecomPropsWithoutItems, ecomPropsMapping,
  constantProps) });
  }
  ...
  if (mandatoryProps) {
  for (let index = 0; index < mandatoryProps.length; index++) { if (!element.data.hasOwnProperty(mandatoryProps[index]))
    { missingMandatoryProps.error=true; missingMandatoryProps.props.push(mandatoryProps[index]); } } } if
    (missingMandatoryProps.error) { finalEvents.splice(index, 1); log('GTM Piano Analytics Tag Template -
    Event "' + element.name + '" aborted, mandatory parameter missing: ', missingMandatoryProps.props);
        }
      }

      log(' GTM Piano Analytics Tag Template - eCommerce bridge'); if (JSON.stringify(finalEvents !=='[]' ))
    dataLayerPush(['sendEvents', finalEvents]); } data.gtmOnSuccess(); } }; const sdkSrc=confObject.sdkSrc
    || "https://tag.aticdn.net/piano-analytics.js" ; injectScript(sdkSrc, pixel.init, data.gtmOnFailure, 'pixelPA' );
    function checkVarPrefix(val, prefixes, splitter) { const valPrefix=val.split(splitter)[0]; for (let index=0; index <
    prefixes.length; index++) { if(prefixes[index]==valPrefix) return true; } return false; } function
    mapProperties(input, mapping, constantProps) { let output=JSON.parse(JSON.stringify(input));
    Object.keys(output).forEach(function (key) { if (mapping.hasOwnProperty(key) && key !==mapping[key]) { let
    outputValue=output[key]; switch (mapping[key]) { case "transaction_promocode" : if (getType(outputValue) !=="array"
    ) outputValue=outputValue.split(","); break; case "product_discount" : if (getType(outputValue) !=="boolean" )
    outputValue=true; break; case "transaction_id" : case "product_id" : case "cart_id" : case "product" :
    case "product_brand" : case "product_category1" : case "product_category2" : case "product_category3" :
    case "product_category4" : case "product_variant" : case "cart_currency" : outputValue=makeString(outputValue) ||
    outputValue; break; case "product_pricetaxincluded" : case "product_pricetaxfree" : case "cart_turnovertaxincluded"
    : case "cart_turnovertaxfree" : case "shipping_costtaxincluded" : case "shipping_costtaxfree" :
    case "product_quantity" : case "cart_quantity" : case "cart_creation_utc" : case "cart_nbdistinctproduct" :
    outputValue=makeNumber(outputValue) || outputValue; break; default: break; } output[mapping[key]]=outputValue;
    Object.delete(output, key); } if (JSON.stringify(constantProps) !=='{}' ) {
    Object.keys(constantProps).forEach(function (key) { output[key]=constantProps[key]; }); } });
    Object.keys(mapping).forEach(function (key) { if (key.indexOf("+") !==-1) { let multipleProps; if (key.indexOf("++")
    !==-1) { multipleProps=key.split("++"); } else { // needed for backward compatibility multipleProps=key.split("+");
    } output[mapping[key]]=input[multipleProps[0]]; for (let i=1; i < multipleProps.length; i++) {
    output[mapping[key]]=output[mapping[key]] + input[multipleProps[i]]; } } if (key.indexOf("--") !==-1) { const
    multipleProps=key.split("--"); output[mapping[key]]=input[multipleProps[0]]; for (let i=1; i < multipleProps.length;
    i++) { output[mapping[key]]=output[mapping[key]] - input[multipleProps[i]]; } } if (key.indexOf("||") !==-1) { const
    multipleProps=key.split("||"); for (let i=0; i < multipleProps.length; i++) { if (input[multipleProps[i]]) {
    output[mapping[key]]=input[multipleProps[i]]; break; } } } }); return output; } const
    DEFAULT_ECOMMERCE_EVENTS_MAPPING={ "view_item_list" : "product.display" , "view_item" : "product.page_display"
    , "add_to_cart" : "product.add_to_cart" , "remove_from_cart" : "product.remove_from_cart" , "view_cart"
    : "cart.display" , "add_shipping_info" : "cart.delivery" , "add_payment_info" : "cart.payment" , "purchase"
    : "transaction.confirmation" , "view_promotion" : "publisher.impression" , "select_promotion" : "publisher.click" ,
    }; const DEFAULT_ECOMMERCE_PRODUCTS_MAPPING={ "transaction.confirmation" : "product.purchased" , "cart.display"
    : "product.display" , "publisher.impression" : "publisher.impression" , "publisher.click" : "publisher.click" , };
    const DEFAULT_ECOMMERCE_PROPS_MAPPING={ "transaction_id" : "transaction_id" , "value" : "cart_turnovertaxincluded"
    , "value--tax" : "cart_turnovertaxfree" , "shipping" : "shipping_costtaxincluded" , "currency||items.currency"
    : "cart_currency" , "coupon" : "transaction_promocode" , "creative_name" : "onsitead_variant" , "creative_slot"
    : "onsitead_category" , "promotion_id" : "onsitead_campaign" , "promotion_name" : "onsitead_creation"
    , "items.item_id" : "product_id" , "items.item_name" : "product" , "items.coupon" : "product_discount"
    , "items.discount" : "product_discount" , "items.item_brand" : "product_brand" , "items.item_category"
    : "product_category1" , "items.item_category2" : "product_category2" , "items.item_category3" : "product_category3"
    , "items.item_category4" : "product_category4" , "items.item_variant" : "product_variant" , "items.price"
    : "product_pricetaxfree" , "items.quantity" : "product_quantity" , }; const
    ECOMMERCE_MANDATORY_PROPERTIES={ "product.display" : ["product_id"], "product.page_display" :
    ["product_id"], "product.add_to_cart" : ["product_id"], "product.remove_from_cart" : ["product_id"], "cart.creation"
    : ["cart_id"], "cart.display" : ["cart_id"], "cart.update" : ["cart_id"], "cart.delivery" :
    ["cart_id"], "cart.payment" : ["cart_id"], "transaction.confirmation" : ["cart_id", "transaction_id"
    ], "product.purchased" : ["cart_id", "product_id" , "transaction_id" ], }; // Use
    https://github.com/gtm-templates-simo-ahava/data-layer-picker to retrieve actually pushed DL values function
    retrieveActualPush(type, value) { const gtmId=data.gtmEventId; const dataLayer=copyFromWindow('dataLayer'); const
    dataType=type || "object" ; const dataValue=value || "" ; const get=(obj, path, def)=> {
    path = path.split('.');
    let current = obj;

    for (let i = 0; i < path.length; i++) { if (!current[path[i]]) return def; current=current[path[i]]; } return
      current; }; if (dataLayer && gtmId) { // Get object from dataLayer that matches the gtm.uniqueEventId let
      obj=dataLayer.map(o=> {
